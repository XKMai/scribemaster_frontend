...
const NoteContextMenu = ({
  ...
  const renameNote = async () => {
    ...
    try {
      const updatedNote = await apiService.updateNote(note.refId, {
        title: renameValue,
      });
  ...
  const deleteNote = async () => {
    const confirmed = window.confirm(
      `Are you sure you want to delete the note "${note.data.title}"? This cannot be undone.`
    );
    if (!confirmed) return;
        ...
        {renaming && (
          <div className="space-y-1">
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button size="sm" onClick={renameNote}>
                Rename
              </Button>
            </div>
          </div>
